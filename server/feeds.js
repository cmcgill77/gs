...
Meteor.methods({
    ...
    'addVerificationData': function(verificationData) {
        ...
        Posts.update(verificationData.postId, {
            $set: {
                verified: true,
        ...
        var owningSource = Sources.find({
            $and: [{
                author: verificationData.verifiedPostAuthor
            }, {
                type: verificationData.verifiedPostType
            }]
        }).fetch();
        console.log("============================")
        ...
        Sources.update(owningSource[0]._id, {
            $push: {
                "levelOfConfidence.verifiedPosts": {
        ...
        Sources.update(owningSource[0]._id, {
            $set: {
                "levelOfConfidence.locValue": newLocValue
            }
        });
    ...
    'twitterQuery': function(selectedFeed) {
        ...
        if (geo[0].geo === false) {
            Twit.get('search/tweets', {
                q: q[0].query,
                count: 100
            }, Meteor.bindEnvironment(function(error, tweets, response) {
        ...
        // if the feed was location based then make a twitter request using the optioal 'geocode' parameters as well
            ...
            Twit.get('search/tweets', {
                q: q[0].query,
                geocode: latitude[0].latitude + ',' + longitude[0].longitude + ',' + radius[0].radius / 1000 + 'km',
                count: 50
            }, Meteor.bindEnvironment(function(error, tweets, response) {
                ...
                for (i = 0; i < tweets.statuses.length; i++) {
                    ...
                    if (dupCheck === 0) {
                        ...
                        if (tweets.statuses[i].coordinates === null) {
                            ...
                            var locCheck = locationCache.find({
                                locationName: place
                            }).fetch();
